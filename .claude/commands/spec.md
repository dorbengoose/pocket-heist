...
argument-hint: "[Short feature description, optional: 'vercel: <component-link>']"
...
- An Optional Vercel design note, if a vercerl design link or code is present
...
4. `vercel_hint` (optional)
- If `$ARGUMENTS` contains the substring `vercel:`
- Then the text after `vercel` is the vercel component link.
- Trim whitespace.
- Example input:
   -`/spec Card Component, vercel: https://www.vercel.com/design/some-link`
   -`vercel_hint` becomes `https://www.figma.com/design/some-link`. 

If you cannot infer a sensible `feature_title` and `feature_slug`, ask the user to clarify instead of guessing.

## Step 2.5 Pull Vercel context when needed

If `vercel_hint` is present and Vercel MCP throuhg MCP Context7 are available:

1. Use the Vercel MCP tools to locate the component, layer or frame.
2. Extract only information that is useful for implementation, such as:
   - Dimensions and layout (grid, spacing, alingment)
...
3. Summarise this as 3 to 8 concise bullet points and also leave a link to the vercel component for future lookups.
4. If lookup fails or the tools are not available, record a note like:
   -`"Design reference could not be retrieved. See Vercel manually for details."`
